...
if (!GOOGLE_CREDENTIALS_JSON_STRING) {
  throw new Error("The GOOGLE_CREDENTIALS_JSON_STRING is not set or is empty.");
}

let credentialsObj;
try {
  credentialsObj = JSON.parse(GOOGLE_CREDENTIALS_JSON_STRING);
} catch (error) {
  console.error("Failed to parse GOOGLE_CREDENTIALS_JSON_STRING:", error);
  throw new Error("Invalid JSON format for GOOGLE_CREDENTIALS_JSON_STRING.");
}
...
export async function processVideoQuery(
  query: string,
  url: string
): Promise<string> {
